...
def run_ga(cost_matrix, supply: Dict[int, Point], cap_transbordo: Dict[int, Point], cap_port: Dict[int, Point],
           demand: float, fitness_limit: float, tam_pop: int, elitism: float) -> list:
    ga = Evolution(origens=supply, transbordos=cap_transbordo, portos=cap_port, demand=demand, cost_matrix=cost_matrix)
    # breakpoint()
    [best, t] = ga.run_evo(
                        generation_limit=10000, tam_population=tam_pop,
                        elitism=elitism, fitness_limit=fitness_limit,
                        show_progress=True
                    )
...
def run_tests(fitness_limit: float, test_id: int, tam_pop: int, elitism: float) -> None:
    print('*'*20, f'BEGIN {test_id}','*'*20)
    ...
    mean_gap = 0
    for _ in range(10):
        ga_score = run_ga(cost_matrix=cost_matrix, supply=supply, cap_transbordo=cap_transbordo,
                                    cap_port=cap_port,demand=demand, fitness_limit=fitness_limit,
                                    tam_pop=tam_pop, elitism=elitism)
        if ga_score < best_score:
            best_score = ga_score
            # graph = ga_graph
        gap = abs((ga_score - fitness_limit) / ((ga_score + fitness_limit) / 2) * 100)
        mean_gap += gap
        print(f'genetic algorithm fitness {ga_score} with {gap:.4f}% gap')
        all_scores.append(ga_score)
        media += ga_score
    print(f'resultado medio: {media/10:.2f}\ngap medio: {mean_gap/10:.2f}')
    
    file_ans = open('D:/Bibliotecas/Documents/IC/Progs/genetic_algo/result_testes/instancia_' + str(test_id) + '.txt', 'w')
    file_ans.write(f'resultado medio: {media/10:.4f}\ngap medio: {mean_gap/10:.2f}\n')
    file_ans.write(f'melhor resultado: {best_score:.4f}\ngap min: {abs((best_score - fitness_limit) / ((best_score + fitness_limit) / 2) * 100):.4f}\n')
    file_ans.close()

    # df = pd.DataFrame()
    # df['scores'] = all_scores
    # df.to_excel('./result_testes/instancia_' + str(test_id) + '.xlsx', index=False)

    print('*'*20, f'END {test_id}','*'*20)
    print()
