...
def main():
    ...
    demmand = [SystemRandom().randint(100, sum_supply) for _ in range(port)]
    sum_demmand = sum(demmand)

    if sum_demmand < sum_supply:
        for i in range(port):
            demmand[i] += abs(sum_supply - sum_demmand) // port
        demmand[0] += abs(sum_supply - sum_demmand) % port
    aux = np.array(demmand)
    pd.DataFrame(aux).to_csv("./dados/demmand.csv", index=None)
